...
int main(int argc, char *argv[]) {
    ...
    while ( i < argc ) {
        if ( strcmp(argv[i], "-i") == 0 ) {
            ...
            if ( !isNumber(strlen(argv[i+1]), argv[i+1]) ) {
                fprintf(stderr, "Not an acceptable input for the semicircle's centre.\n");
                return 1;
            }
            x = atof(argv[i+1]);
            ...
            if ( !isNumber(strlen(argv[i+3]), argv[i+3]) ) {
                fprintf(stderr, "Not an acceptable input for the semicircle's rad.\n");
                return 1;
            }
            r = atof(argv[i+3]);
            if ( i+4 >= argc ) { fprintf(stderr, "Error ! Wrong input of arguments\n"); return 1;  }
            if ( strcmp(argv[i+4], "N") && strcmp(argv[i+4], "W") && strcmp(argv[i+4], "S") && strcmp(argv[i+4], "E") ) {
                fprintf(stderr, "Not an acceptable input for the semicircle's position.\n");
                return 1;
            }
            pos = argv[i+4];
            i+= 5;
    ...
    for (i = 0; i < numOfrecords; i++) {
        ...
        if (strcmp(pos, "N") == 0) {
            if (powf(rec.x - x, 2.0)+powf(rec.y - y, 2.0) <= powf(r, 2.0) && rec.y >= y) {
        ...
        else if (strcmp(pos, "E") == 0) {
            if (powf(rec.x - x, 2.0)+powf(rec.y - y, 2.0) <= powf(r, 2.0) && rec.x >= x) {
                fprintf(write_fp, "%.2f\t%.2f\n",rec.x, rec.y );
                fflush(write_fp);
            }
        }
        else if (strcmp(pos, "W") == 0) {
            if (powf(rec.x - x, 2.0)+powf(rec.y - y, 2.0) <= powf(r, 2.0) && rec.x <= x) {
                fprintf(write_fp, "%.2f\t%.2f\n",rec.x, rec.y );
                fflush(write_fp);
            }
