...
int main(int argc, char *argv[]) {
    ...
    while ( i < argc ) {
        if ( strcmp(argv[i], "-i") == 0 ) {
            ...
            if ( !isNumber(strlen(argv[i+1]), argv[i+1]) ) {
                fprintf(stderr, "Not an acceptable input for the ellipse's centre.\n");
                return 1;
            }
            h = atof(argv[i+1]);
            ...
            if ( !isNumber(strlen(argv[i+3]), argv[i+3]) ) {
                fprintf(stderr, "Not an acceptable input for the ellipse's big axis.\n");
                return 1;
            }
            a = atof(argv[i+3]);
            if ( i+4 >= argc ) { fprintf(stderr, "Error ! Wrong input of arguments\n"); return 1;  }
            if ( !isNumber(strlen(argv[i+4]), argv[i+4]) ) {
                fprintf(stderr, "Not an acceptable input for the ellipse's small axis.\n");
                return 1;
            }
            b = atof(argv[i+4]);
            i+= 5;
    ...
    for (i = 0; i < numOfrecords; i++) {
        ...
        if (powf(rec.x - h, 2.0)/powf(a, 2.0)+powf(rec.y - k, 2.0)/powf(b, 2.0) <= 1) {
