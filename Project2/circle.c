...
int main(int argc, char *argv[]) {
    ...
    i = 1;
    while( i < argc ) { //Checking the in-line arguments
        if ( strcmp(argv[i], "-i") == 0 ) {
            ...
            if ( !isNumber(strlen(argv[i+1]), argv[i+1]) ) {
                fprintf(stderr, "Error ! Not an acceptable input for the cycle's centre.\n");
                return 1;
            }
            x = atof(argv[i+1]);
            ...
            if ( !isNumber(strlen(argv[i+3]), argv[i+3]) ) {
                fprintf(stderr, "Error ! Not an acceptable input for the cycle's rad.\n");
                return 1;
            }
            r = atof(argv[i+3]);
            i+= 4;
    ...
    if (flagN && pointsToReadCount <= numOfrecords && pointsToReadCount > 0) {    //We check if there is a -n flag on input and if the pointsToReadCount requested
        ...
        fprintf(stderr, "Error ! pointsToReadCount %d, bytesToSkip%d.\n", pointsToReadCount, bytesToSkip);
        return 1;
    }
    for (i = 0; i < numOfrecords; i++) {
        fread(&rec, sizeof(rec), 1, read_fp);
        if (powf(rec.x - x, 2.0)+powf(rec.y - y, 2.0) <= powf(r, 2.0)) {
