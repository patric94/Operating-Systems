...
int main(int argc, char *argv[]) {
    ...
    while( i < argc ) {
        if ( strcmp(argv[i], "-i") == 0 ) {
            ...
            if ( !isNumber(strlen(argv[i+1]), argv[i+1]) ) {
                fprintf(stderr, "Not an acceptable input for the square's centre.\n");
                return 1;
            }
            x = atof(argv[i+1]);
            ...
            if ( !isNumber(strlen(argv[i+3]), argv[i+3]) ) {
                fprintf(stderr, "Not an acceptable input for the square's rad.\n");
                return 1;
            }
            r = atof(argv[i+3]);
            i+= 4;
    ...
    for (i = 0; i < numOfrecords; i++) {
        ...
        if (fabsf(rec.x - x)+fabsf(rec.y - y) <= r) {
